...
| 功能 | 说明 | 工作量 | 状态 |
|------|------|--------|------|
| **TaskOutput 工具** | 获取后台代理结果 | 2天 | ✅ 已完成 |
| 模型 inherit 继承 | Agent继承主对话模型 | 1天 | ✅ 已完成 |
| Agent Mention 解析 | @agent-type 语法 | 1天 | 待实现 |
| 权限模式执行 | plan/acceptEdits/bypass | 2天 | 待实现 |
| 超时管理 | Agent 执行超时控制 | 1天 | 待实现 |

### 3. Bash 工具
| 功能 | 说明 | 工作量 | 状态 |
|------|------|--------|------|
| **stdin 输入处理** | 管道输入到命令 | 1-2天 | 待实现 |
| **Windows Jobber** | 后台进程隔离 | 3-5天 | 待实现 |
| 时间字符串解析 | "2h", "30m" 格式 | 1天 | ✅ 已完成 |
| 实时输出回显 | echoOutput 选项 | 1天 | ✅ 已完成 |
...
| 功能 | 说明 | 工作量 | 状态 |
|------|------|--------|------|
| **MCPB 文件支持** | 官方扩展包格式 | 2-3周 | 待实现 |
| 输出限制 | MAX_MCP_OUTPUT_TOKENS | 1天 | ✅ 已完成 |
| 完整配置验证 | 环境变量展开 ${VAR} | 1周 | 待实现 |
